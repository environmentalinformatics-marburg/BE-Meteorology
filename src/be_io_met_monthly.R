...
be_io_met_monthly <- function(df_met, ...){
  ...
  df_met$g_belcm <- paste0(substr(as.character(df_met$plot), 1, 3),
                          "_", substr(as.character(df_met$datetime), 6, 7))
  
  # Define grouping by plot and month
  df_met$g_pm <- paste0(as.character(df_met$plot), "_",
...
be_io_met_monthly_urlData <- function(df_met){
  # df_met <- read.table(filepath, header = TRUE, sep = ",", dec = ".")
